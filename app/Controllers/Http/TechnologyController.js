...
class TechnologyController {
    async index({ response, auth, view, params }) {
        ...
        if (user.role == "adm") {
            ...
            const technologies = await Database.select('technologies.*'
            , Database.raw('COUNT(DISTINCT projects.id) as projects')
            , Database.raw('COUNT(DISTINCT errors.id) as errors'))
            .from('technologies').leftJoin('tech_pros', 'tech_pros.technology_id', 'technologies.id')
            .leftJoin('projects', 'tech_pros.project_id', 'projects.id')
            .leftJoin('errors', 'errors.technology_id', 'technologies.id').groupBy('technologies.id')
            var i = user.img
    ...
    async create({ response, auth, view, params }) {
        ...
        if (user.role == "adm") {
            var i = user.img
            var n = user.firstname + " " + user.familyname
            return view.render("dashboard.technology.create", { user: user, img: i, myname: n });
        } else if (user.role == "int" || user.role == "emp") {
            return response.redirect("/index");
        } else {
            return view.render("inv.index");
    ...
    async edit({ resolve, auth, view, params }) {
        ...
        if (user.role == "adm") {
            ...
            const technologies = await Database.from('technologies').where('technologies.id', technology_id)
