...
class ModuleController {
    async index({ response, auth, view, params }) {
        ...
        if (user.role == "adm" || user.role == "int" || user.role == "emp") {
            ...
            const tasks = await Database.select('tasks.*', Database.raw('projects.name as project'),
                    Database.raw('GROUP_CONCAT(CONCAT(users.firstname, " ",users.familyname)) as subteam'))
                ...
                .where('projects.id', id_project).groupBy('tasks.id')
                /* *** */
            const projects = await Database.select('projects.*',
                    Database.raw('GROUP_CONCAT(distinct CONCAT(u2.firstname, " ", u2.familyname, " ") ) as members'),
                ...
                .leftJoin('technologies', 'tech_pros.technology_id', 'technologies.id').where('projects.id', id_project)
            ...
            if (start > current) {
                ...
                if (project.actual_date == undefined || project.actual_date == null) {
                    ...
                    var finished = await Database.select(Database.raw('SUM(weight) as val'))
                        .from('sub_tasks').innerJoin('tasks', 'tasks.id', 'sub_tasks.task_id')
                        .innerJoin('projects', 'tasks.project_id', 'projects.id').where('projects.id', project.id)
                        .whereNotNull('sub_tasks.actual_date').groupBy('projects.id')
                    var total = await Database.select(Database.raw('SUM(weight) as val'))
                        .from('sub_tasks').innerJoin('tasks', 'tasks.id', 'sub_tasks.task_id')
                        .innerJoin('projects', 'tasks.project_id', 'projects.id')
                        .where('projects.id', project.id).groupBy('projects.id')
                    var f
            ...
            for (var i = 0; i < tasks.length; i++) {
                ...
                if (start > current) {
                    ...
                    if (tasks[i].end_date == undefined || tasks[i].end_date == null) {
                        ...
                        if (tasks[i].actual_date == undefined || tasks[i].actual_date == null) {
    ...
    async create({ response, auth, view, params }) {
        const user = await auth.getUser()
        if (user.role == "adm" || user.role == "emp") {
            const id_project = params.id;
            ...
            const users = await Database.select('users.*').from('users')
                ...
                .innerJoin('projects', 'projects.team_id', 'teams.id').where('projects.id', id_project)
            ...
            return view.render("dashboard.module.create", { users: users, team: team[0].name, id: id_project, img: i, myname: n, myrole: r });
    ...
    async validate({ response, auth, view, params }) {
        ...
        if (user.role == "adm" || user.role == "emp") {
            ...
            return view.render("dashboard.module.validate", { module_id: module_id, id_pro: id_pro, img: i, myname: n, myrole: r });
        } else if (user.role == "int") {
            return response.redirect("/index");
        } else {
            return view.render("inv.index");
    ...
    async finish({ request, auth, view, params, response }) {
            ...
            return response.redirect("/project/" + id_pro + "/module/" + module_id + "/task");
        }
        /************************************************************************** */
    ...
    async edit({ auth, view, params }) {
        ...
        if (user.role == "adm") {
            ...
            .select('tasks.*',Database.raw('GROUP_CONCAT(task_users.user_id) as team'))
            .from('tasks').innerJoin('task_users', 'task_users.task_id', 'tasks.id')
    ...
    async update({ request, response, auth, params }) {
        ...
        const { name, description, start_date, end_date, team, oldteam } = request.all();
        const module_id = params.module_id;
        ...
        for (var i = 0; i < to_del.length; i++) {
            ...
            const taskUser = await TaskUser.findBy({'user_id':to_del[i],'task_id':modules.id});
