...
class DashboardController {
  async index({ auth, view }) {
    ...
    if(user.role== "adm" || user.role== "int" || user.role== "emp"){
      ...
      const todo = await Database.select('sub_tasks.*').from('sub_tasks')
      .innerJoin('sub_task_users', 'sub_task_users.sub_task_id', 'sub_tasks.id')
      .where('sub_task_users.user_id',user.id ).where('sub_tasks.status',"td")
      .orderBy('sub_tasks.created_at','desc');
      const progress = await Database.select('sub_tasks.*').from('sub_tasks')
      .innerJoin('sub_task_users', 'sub_task_users.sub_task_id', 'sub_tasks.id')
      .where('sub_task_users.user_id',user.id ).where('sub_tasks.status',"pg")
      .orderBy('sub_tasks.created_at','desc');
  
      const completed = await Database.select('sub_tasks.*').from('sub_tasks')
      .innerJoin('sub_task_users', 'sub_task_users.sub_task_id', 'sub_tasks.id')
      .where('sub_task_users.user_id',user.id ).where('sub_tasks.status',"fn")
      .orderBy('sub_tasks.actual_date','desc');
      var colors = ["green", "orange", "light-blue", "cyan", "blue-grey","red"]
      var data = []
  ...
  async add({ response, request, auth }) {
    ...
    await task.save().then(function () {
       ...
       req = req + "&token=b9d5a676cf703a60949e0a7746d030135e103acf24e6d153f445fbc1e89c71fd"
      ...
      fetch(req, {method: 'POST'}).then(response => {
        ...
        }).then(text => console.log(JSON.parse(text))).catch(err => console.error(err));
    }).catch(e => {
      return response.status(500).json({"error":e})
    });
