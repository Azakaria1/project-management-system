...
class ProjectController {
    async index({ response, auth, view, params }) {
        ...
        if (user.role == "adm") {
            ...
            var tech = ''
            var projects =  Database.select('projects.*', Database.raw('teams.name as team'), Database.raw('CONCAT(users.firstname, " ",users.familyname) as leader'))
            ...
            if(tech_id != undefined && tech_id != null){
                projects = projects.innerJoin('tech_pros', 'tech_pros.project_id', 'projects.id')
                .innerJoin('technologies', 'tech_pros.technology_id', 'technologies.id')
                .where('technologies.id',tech_id).groupBy('projects.id')
                const technologies = await Database.from('technologies').where('technologies.id',tech_id)
                ...
                if (start > current) {
                    ...
                    if (projects[i].actual_date == undefined || projects[i].actual_date == null) {
                        var finished = await Database.select(Database.raw('SUM(weight) as val'))
                            ...
                            .innerJoin('projects', 'tasks.project_id', 'projects.id').where('projects.id', projects[i].id)
                        ...
                        var total = await Database.select(Database.raw('SUM(weight) as val'))
                            ...
                            .where('projects.id', projects[i].id).groupBy('projects.id')
                        var f
    ...
    async myIndex({ response, auth, view, params }) {
        ...
        if (user.role == "adm" || user.role == "int" || user.role == "emp") {
            const projects = await Database.select('projects.*', Database.raw('teams.name as team'),
                    Database.raw('CONCAT(users.firstname, " ",users.familyname) as leader'))
                .from('projects').innerJoin('teams', 'projects.team_id', 'teams.id')
                .innerJoin('users', 'projects.leader_id', 'users.id')
                .innerJoin('team_users', 'team_users.team_id', 'teams.id')
                .where('users.id',user.id).orWhere('team_users.user_id', user.id)
                .groupBy('projects.id')
            ...
            for (var i = 0; i < projects.length; i++) {
                ...
                projects[i].status = status
                if(projects[i].description != undefined && projects[i].description != null){
    ...
    async create({ response, auth, view, params }) {
        ...
        if (user.role == "adm" || user.role == "emp") {
            ...
            return view.render("dashboard.project.create", { users: users, teams: teams, technologies: technologies, img: i, myname: n, myrole: r });
    ...
    async store({ request, response, auth }) {
            ...
            const { type, name, git_link, start_date, end_date, leader_id, team_id, technologies, description } = request.all();
    ...
    async validate({ response, auth, view, params }) {
        ...
        if (user.role == "adm" || user.role == "emp") {
            ...
            return view.render("dashboard.project.validate", { project_id: project_id, img: i, myname: n, myrole: r });
        } else if (user.role == "int") {
            return response.redirect("/index");
        } else {
            return view.render("inv.index");
    ...
    async edit({ resolve, auth, view, params }) {
        ...
        if (user.role == "adm") {
            ...
            .select('projects.*',Database.raw('GROUP_CONCAT(technologies.id) as tech')).from('projects')
    ...
    async update({ request, response, auth, params }) {
        ...
        const { name, start_date, end_date, description, type, git_link, img, team_id, leader_id,technologies,oldtech  } = request.all();
        const project_id = params.project_id;
        ...
        for (var i = 0; i < to_del.length; i++) {
            ...
            const techPro = await TechPro.findBy({'technology_id':to_del[i],'project_id':project_id});
            await techPro.delete()
