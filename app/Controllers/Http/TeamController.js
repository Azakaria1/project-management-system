...
class TeamController {
  async index({ response, auth, view, params }) {
    ...
    if(user.role== "adm"){
      ...
      const Teams = await Database.select('teams.*',Database.raw('GROUP_CONCAT(CONCAT(users.firstname, " ", users.familyname, " ") ) as members'))
      .from('teams').leftJoin('team_users', 'team_users.team_id', 'teams.id')
      .leftJoin('users', 'team_users.user_id', 'users.id').groupBy('teams.id')
      var i = user.img
      var n = user.firstname + " " + user.familyname
      return view.render("dashboard.team.index", {
        Teams: Teams,user:user,img:i,myname:n
      });
    }
  ...
  async myteam({ response, auth, view, params }) {
    ...
    if(user.role== "adm" || user.role== "int" || user.role== "emp" ){
        ...
        .from('teams').leftJoin('team_users', 'team_users.team_id', 'teams.id')
        .leftJoin('users', 'team_users.user_id', 'users.id').where('users.id',user.id).groupBy('teams.id')
      const Teams = await Database.select('teams.*',
      Database.raw('GROUP_CONCAT(CONCAT(users.firstname, " ", users.familyname, " ") ) as members'))
      .from('teams').leftJoin('team_users', 'team_users.team_id', 'teams.id')
      .leftJoin('users', 'team_users.user_id', 'users.id').whereIn('teams.id', subquery).groupBy('teams.id')
      var i = user.img
      var n = user.firstname + " " + user.familyname
      var r
      var int
      ...
      return view.render("dashboard.team.index", {
        Teams: Teams,user:user,img:i,myname:n,myrole: r, restriction:int
      });
    ...
    async create({ response, auth, view, params }) {
        ...
        if (user.role == "adm") {
            ...
            var i = user.img
            var n = user.firstname + " " + user.familyname
            return view.render("dashboard.team.create", { users: users, user: user, img: i, myname: n });
        } else if (user.role == "int" || user.role == "emp") {
            return response.redirect("/index");
        } else {
            return view.render("inv.index");
    ...
    async edit({ resolve, auth, view, params }) {
        ...
        if (user.role == "adm") {
            ...
            .select('teams.*',Database.raw('GROUP_CONCAT(team_users.user_id) as users'))
            .from('teams').leftJoin('team_users', 'team_users.team_id', 'teams.id')
    ...
    async update({ request, response, auth, params }) {
        ...
        for (var i = 0; i < to_del.length; i++) {
            ...
            const tu = await TeamUsers.findBy({'user_id':to_del[i],'team_id':teams.id});
            await tu.delete()
