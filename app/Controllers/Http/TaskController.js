...
class TaskController {
  async index({ response, auth, view, params }) {
    ...
    if(user.role== "adm" || user.role== "int" || user.role== "emp"){
      ...
      const tasks = await Database.from('sub_tasks').where('sub_tasks.task_id',id_mod )
      console.log(tasks)
      /* *** */
      var module = await Database.select('tasks.*',Database.raw('projects.name as project'))
      .from('tasks').innerJoin('projects', 'projects.id', 'tasks.project_id')
      ...
      for(var i=0;i < tasks.length;i++){
        ...
        if(tasks[i].status == 'td'){
          tasks[i].status = "<span class='badge badge-soft-info font-size-12'>To do</span>"
        }
        else if(tasks[i].status == 'pg'){
          tasks[i].status = "<span class='badge badge-soft-warning font-size-12'>In progress</span>"
        }
        else if(tasks[i].status == 'fn'){
          tasks[i].status = "<span class='badge badge-soft-success font-size-12'>Finished</span>"
          tasks[i].bool = 1
        }
        else{
          tasks[i].status = ""
  ...
  async details({ response, auth, view, params }) {
    ...
    if(user.role== "adm" || user.role== "int" || user.role== "emp"){
      ...
      var task = await Database.select('sub_tasks.*',Database.raw('COUNT(errors.id) as errors'),
      Database.raw('projects.name as project'),Database.raw('tasks.name as module'))
      .from('sub_tasks').leftJoin('errors', 'errors.subtask_id', 'sub_tasks.id')
      .leftJoin('tasks', 'tasks.id', 'sub_tasks.task_id')
      .leftJoin('projects', 'projects.id', 'tasks.project_id')
      .where('sub_tasks.id',task_id ).groupBy('sub_tasks.id')
    ...
    var n = user.firstname + " " + user.familyname
      ...
      return view.render("dashboard.task.details", {
        task: task,
        task_id:task_id,img:i,myname:n,myrole:r
      });
    }
  ...
  async validate({ response, auth, view, params }) {
    ...
    if(user.role== "adm" || user.role== "int" || user.role== "emp" ){
      ...
      return view.render("dashboard.task.validate",{task_id:task_id,img:i,myname:n,myrole:r});
    }
  ...
  async create({ response, auth, view, params }) {
    ...
    if(user.role== "adm" || user.role== "int" || user.role== "emp" ){
      const id_mod= params.id_mod;
      const id_pro = params.id_pro;
      const users = await Database.select('users.*').from('users')
      .innerJoin('team_users', 'team_users.user_id', 'users.id')
      .innerJoin('teams', 'team_users.team_id', 'teams.id')
      .innerJoin('projects', 'projects.team_id', 'teams.id').where('projects.id',id_pro)
      const team = await Database.select('teams.name').from('teams')
      .innerJoin('projects', 'projects.team_id', 'teams.id').where('projects.id',id_pro)
      var i = user.img
      var n = user.firstname + " " + user.familyname
      var r 
      ...
      return view.render("dashboard.task.create",
       {users: users,team:team[0].name,id_pro: id_pro,id_mod: id_mod,img:i,myname:n,myrole:r });
    }
  ...
  async store({request, response,auth,params}){
    const user = await auth.getUser()
      ...
      const {  name, start_date, end_date, team, description, commit, start_time, end_time,weight} = request.all();
      const id_mod= params.id_mod;
      const id_pro = params.id_pro;
      ...
      return response.redirect("/project/"+ id_pro + "/module/"+ id_mod +"/task");
...
async index2({ response, auth, view, params }) {
  ...
  if(user.role != 'adm'){
    ...
    return view.render("dashboard.task.index", {
      tasks: tasks,img:i,myname:n,myrole:r
    });
  }
...
async search({ request, auth, view, params }) {
  ...
  if(user.role== "adm" || user.role== "int" || user.role== "emp"){
    ...
    var query = Database.select('sub_tasks.*').from('sub_tasks')
    .innerJoin('sub_task_users', 'sub_task_users.sub_task_id', 'sub_tasks.id')
    .where('sub_task_users.user_id',user.id )
      ...
      if(weight== 0 && state!='all'){
        ...
        query = query.where('sub_tasks.weight',weight ).where('sub_tasks.status', state)
        }
...
async create2({ response, auth, view, params }) {
  ...
  if(user.role== "adm" || user.role== "int" || user.role== "emp" ){
    ...
    return view.render("dashboard.task.create",{img:i,myname:n,myrole:r});
  }
...
async store2({request, response,auth,params}){
  const user = await auth.getUser()
  if(user.role== "adm" || user.role== "int" || user.role== "emp" ){
    const {  name, start_date, end_date, description, commit, start_time, end_time, weight} = request.all();
    const task = new SubTask();
    if(name!='' && name != undefined){
      task.name = name;
    }
...
async delete({ response, request, view, params }) {
  ...
  async edit({ response, auth, view, params }) {
    ...
    if(user.role== "adm" || user.role== "int" || user.role== "emp"){
      const task_id= params.task_id;
      const sub_tasks = await Database.from('sub_tasks').where('sub_tasks.id',task_id)
