...
class ErrorController {
  async index({ response, auth, view, params }) {
    ...
    if(user.role== "adm" || user.role== "int" || user.role== "emp" ){
      const errors = await Database.select('errors.*',Database.raw('technologies.name as technology')).
      from('errors').innerJoin('technologies','technologies.id','errors.technology_id').where('errors.user_id',user.id)
...
async details({ response, auth, view, params }) {
  ...
  if(user.role== "adm" || user.role== "int" || user.role== "emp"){
    ...
    const errors = await Database.select('errors.*',
    Database.raw('CONCAT(users.firstname, " ", users.familyname, " ") as user'),
    Database.raw('technologies.name as technology'),
    Database.raw('sub_tasks.name as task')).
    from('errors').innerJoin('technologies','technologies.id','errors.technology_id')
    .innerJoin('sub_tasks','sub_tasks.id','errors.subtask_id')
    .innerJoin('users','users.id','errors.user_id').where('errors.id',error_id)
    for(var i=0;i < errors.length;i++){
      errors[i].error_date = dateMaker(errors[i].error_date)
      if(errors[i].is_resolved==1){
        errors[i].status = "resolved"
...
async index2({ response, auth, view, params }) {
  ...
  if(user.role== "adm" || user.role== "int" || user.role== "emp" ){
    ...
    const errors = await Database.select('errors.*',Database.raw('CONCAT(users.firstname, " ", users.familyname, " ") as user')).
    from('errors').innerJoin('users','users.id','errors.user_id').where('errors.user_id',user.id).where('errors.subtask_id',task_id)
    for(var i=0;i < errors.length;i++){
      errors[i].error_date = dateMaker(errors[i].error_date)
      if(errors[i].is_resolved==1){
...
/************************************************ */
  async create({ response, auth, view, params }) {
    ...
    if(user.role== "adm" || user.role== "int" || user.role== "emp"){
      ...
      {technologies:technologies,task_id:task_id,img:i,myname:n,myrole:r});
    }
...
async solving({ response, auth, view, params }) {
  ...
  if(user.role== "adm" || user.role== "int" || user.role== "emp"){
    ...
    return view.render("dashboard.error.solve",{error_id:error_id,img:i,myname:n,myrole:r});
  }
...
/************************************************* */
  async store({request, response,auth,params}){
    ...
    var { name, commit, error_date, technology, resolve, description, is_resolved} = request.all();
